package Exception_task3.Presenter;

import Exception_task3.Model.DataChecker;
import Exception_task3.Model.ParsingDataStringException;
...
public class Presenter< V extends View> {
    ...
    private Exception_task3.Model.DataChecker model;
    private V view;
    ...
    public void start() {
        ...
        do {
            ...
            if (input.equals("Exit")) {
                ...
                if (inputDataCount == -1) {
                    view.printOutput("Слишком мало данных на вводе (должно быть " + Exception_task3.Model.DataChecker.dataCount
                            + " разделённых пробелом ' ': Фамилия Имя Отчество НомерТелефона ДатаРождения Пол)\n");
                } else if (inputDataCount == 0) {
                    view.printOutput("Слишком много данных на вводе (должно быть " + Exception_task3.Model.DataChecker.dataCount
                            + " разделённых пробелом ' ': Фамилия Имя Отчество НомерТелефона ДатаРождения Пол)\n");
                } else {
                    try {
